...
enum ECBFormCtrlEventsIIII
{
	...
	//   此枚举值将作为 m_hashEventNotify 的各项 Key 的高 2 位
	//   Key 的低两位为控件ID
	eListView_Click = NM_CLICK | c_CBNotifyEventBase,			// void lsv1_Click(int indexItem, int indexSubItem, int x, int y)
																// listview 控件被左键单击，indexItem、indexSubItem 分别表示被单击的条目编号和子条目编号（从1开始）
																// (x,y) 为被单击的位置（控件坐标系）
	eListView_ClickRight = NM_RCLICK | c_CBNotifyEventBase,		// listview 控件被右击  void lsv1_ClickRight(int indexItem, int indexSubItem, int x, int y)
	eListView_DblClick = NM_DBLCLK | c_CBNotifyEventBase,		// listview 控件被双击  void lsv1_DblClick(int indexItem, int indexSubItem, int x, int y)
	eListView_DblClickRight = NM_RDBLCLK | c_CBNotifyEventBase,	// listview 控件被右键双击 void lsv1_DblClickRight(int indexItem, int indexSubItem, int x, int y)
