...
void NumberPress(LPCTSTR szNumber)
{
	if (m_fInNum)		// 此前已按过数字键
	{
		...
		else if(m_sNum1 == TEXT("0"))
			m_sNum1 = szNumber;
		else if(m_sNum1 == TEXT("-0"))
			m_sNum1[1] = szNumber[0];	
		else
...
void OperatorPress(EOperatorType oper)
{
	...
	if (m_operTask){					// 之前有计算任务 if ( m_operTask != 0 )
		...
		m_sNum1 = TEXT("");	// 腾出 m_sNum1 准备接收新的数字字符串
							// 之后待按下  + - * / 或 = 时再予计算
	}		
...
void cmdSign_Click()
{
	//已修改，计算完成后标志位清零程序认为没按过数字键
	//但逻辑上此时应能改变计算结果符号
	//故改为判断m_sNum1是否长度为0
	// 若尚未按过数字键就直接按 +/- 键，则按键无效，直接退出函数
	if (m_sNum1.length() == 0) { MessageBeep(mb_SoundSpeaker); return; }
...
void form1_Load()
{
	...
	form1.Control(ID_cmdMinus).TagIntSet(2);
	form1.Control(ID_cmdMul).TagIntSet(3);
	form1.Control(ID_cmdDiv).TagIntSet(4);
